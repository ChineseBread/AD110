...
function PhoneComponent({UrlList}:{UrlList:BlogLink[]}){
    return(
        <Collapse ghost>
            {UrlList.map(category => {
                ...
                return(
                    <Panel key={url_id} style={{borderBottom:'1px solid #bfbfbf',fontSize:14}} header={
                        <a onClick={() => UserOperation.clickUrl(url_id)} target='_blank' rel='noreferrer' href={url_value}>
                            <span style={{
                                color:'#262626',
                        ...
                        <a onClick={() => UserOperation.clickUrl(url_id)} target='_blank'  rel="noreferrer" href={url_value}><span style={{color:'#8c8c8c'}}>{url_info}</span></a>
                    </Panel>
                )
            })}
        </Collapse>
...
function Extra({url_likes,url_id,url_hits,iscool_switch}:BlogLink){
