...
function PhoneHeader({Hot,Articles,News,Recruit}:Props) {
    return (
...
function NewsCarousel({Hot}:{Hot:BlogData[]}){
    return(
...
function NewsVertical({Articles}:{Articles:BlogData[]}){
    return (
        <div className={styles.right_container}>
            {Articles.map(({log_id,log_title,log_cover_image,log_poster_image}) => {
                return (
                    <Link key={log_id} href={`/section?articleid=${log_id}`}>
...
function ConciseNews({News,Recruit}:{News:BlogData[],Recruit:BlogData[]}){
    return(
