...
import {getImageUrl} from "@utils/request/request";
...
function BlogCommentsList({log_id,CommentsList,total}:Props) {
    ...
    const onFinish = ({name,comment,website,email}:any) => {
        UserOperation.sendBlogComment(log_id,name,comment,website,email).then(result => {
            if (result.Ok){
    ...
    const getMoreComments = () => {
        BlogDataRequest.getBlogCommentsByBlogID(log_id,5,page + 1).then(result => {
            if (result.Ok){
                const {CommentsData = {CommentsList:[]}} = result
                setComments(CommentsList => [...CommentsList,...CommentsData.CommentsList])
                setPage(page => page + 1)
            }
        })
    }
